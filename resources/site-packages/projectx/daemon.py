...
from projectx.osarch import PLATFORM, get_platform
from projectx.config import projectxD_HOST
from projectx.addon import ADDON, ADDON_ID, ADDON_PATH
from projectx.util import notify, dialog_ok, system_information, getLocalizedString, getWindowsShortPath
...
def get_projectxd_checksum(path):
...
def clear_fd_inherit_flags():
    # Ensure the spawned projectx binary doesn't inherit open files from Kodi
...
def jsonrpc_enabled(notify=False):
    try:
        ...
        if notify:
            ...
            dialog.ok("projectx", getLocalizedString(30199))
    return False
...
def shutdown():
    try:
        urllib_request.urlopen(projectxD_HOST + "/shutdown")
    except Exception:
        pass

def wait_for_abortRequested(proc, monitor):
    monitor.closing.wait()
    log.info("projectxd: exiting projectxd daemon")
    try:
        if proc is not None:
            proc.terminate()
    except OSError:
        pass  # Process already exited, nothing to terminate
    log.info("projectxd: projectxd daemon exited")

def projectxd_thread(monitor):
    crash_count = 0
    try:
        monitor_abort = xbmc.Monitor()  # For Kodi >= 14
        while not monitor_abort.abortRequested():
            log.info("projectxd: starting projectxd")
            proc = None
            if hasSubprocess:
                proc = start_projectxd(stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                if not proc:
                    break
            else:
                log.info("projectxd: current system is unable to run the binary")
                break
            ...
            if binary_platform["os"] == "windows":
                ...
                # read. We count on the fact that projectx daemon flushes its log
