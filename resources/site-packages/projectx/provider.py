...
from projectx.util import notify, getLocalizedString
from projectx.logger import log
from projectx.config import projectxD_HOST
from projectx.addon import ADDON, ADDON_ID
...
def register(search, search_movie, search_episode, search_season=None):
    ...
    try:
        ...
        try:
            ...
            notify("%s: %s" % (getLocalizedString(30224), repr(e).encode('utf-8')))
            try:
                urllib_request.urlopen("%s/provider/%s/failure" % (projectxD_HOST, ADDON_ID))
            except:
                pass
    finally:
        try:
