...
from projectx.addon import ADDON, ADDON_PATH
from projectx.logger import log
from projectx.config import JSONRPC_EXT_PORT, projectxD_HOST
from projectx.osarch import PLATFORM
from projectx.util import toUtf8, notify, getLocalizedString, getLocalizedLabel, getLocalizedText, system_information
from projectx.overlay import OverlayText
from projectx.dialog_select import DialogSelect
from projectx.dialog_insert import DialogInsert

XBMC_PLAYER = xbmc.Player()

class projectxRPCServer(BaseHandler):
    ...
    def AddonFailure(self, addonId):
        ...
        if self._failures[addonId] > int(ADDON.getSetting("provider_allowed_failures")):
            try:
                ...
                urllib_request.urlopen("%s/provider/%s/disable" % (projectxD_HOST, addonId))
            except:
...
def server_thread():
    try:
        ...
        s = bjsonrpc.createserver(host="0.0.0.0", port=JSONRPC_EXT_PORT, handler_factory=projectxRPCServer)
        log.info("projectx: starting jsonrpc service")
        s.serve()
        log.info("projectx: exiting jsonrpc service")
    except Exception:
        import traceback
